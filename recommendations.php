...
<?php
  ...
  $query = "SELECT item_ID, item_name, pro_desc, picture FROM item WHERE category_ID IN (SELECT category_ID FROM item WHERE item_ID IN (SELECT item_ID FROM view_history WHERE (item_ID IN (SELECT item_ID FROM view_history WHERE view_times = (SELECT MAX(view_times) FROM view_history WHERE buyer_ID = '$user_id')))))";
